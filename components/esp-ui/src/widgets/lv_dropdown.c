...
static void lv_dropdown_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_FOCUSED) {
        ...
        if(c == LV_KEY_RIGHT || c == LV_KEY_DOWN) {
            ...
            else if(dropdown->option_cnt) {
                if(dropdown->sel_opt_id + 1 < dropdown->option_cnt) dropdown->sel_opt_id++;
                else dropdown->sel_opt_id = 0;
                position_to_selected(obj);
            }
        }
            ...
            if(!lv_dropdown_is_open(obj)) {
                ...
                if(dropdown->sel_opt_id > 0) dropdown->sel_opt_id--;
                else dropdown->sel_opt_id = dropdown->option_cnt - 1;
                position_to_selected(obj);
            }
        }
