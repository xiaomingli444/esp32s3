...
static QueueHandle_t xQueueRGBData;
...
void ws2812_task_handler(void *arg)
{
    rgb_struct_t rgb_values;
    ws2812_strip_handle_t ws2812_handle = NULL; //WS2812句柄
    ws2812_init(WS2812_GPIO_NUM,WS2812_LED_NUM,&ws2812_handle);
    while (1)
    {
        if (xQueueReceive(xQueueRGBData, &rgb_values, portMAX_DELAY))
        {
            ESP_LOGI(TAG, "LED Color - R: %d, G: %d, B: %d", rgb_values.red, rgb_values.green, rgb_values.blue);
            ws2812_write(ws2812_handle, 0, rgb_values.red, rgb_values.green, rgb_values.blue);
        }
    }
}
...
{
    xQueueRGBData = RGBData;
    xTaskCreatePinnedToCore(ws2812_task_handler, TAG, 4 * 1024, NULL, 4, NULL, 0);
}